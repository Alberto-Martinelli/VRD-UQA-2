...
## Sample 2 — Layout+Entity Corruption (Spatial + Event) | Gemma4 | BoundingDocs | C2

| Field | Value |
|-------|-------|
| **Model** | Gemma4 |
| **Corruption type** | Layout+Entity (C2) — spatial reference + event type both swapped |
| **Dataset** | BoundingDocs |
| **Complexity** | C2 |
| **Original question** | Where is the **dinner** held? |
| **Corrupted question** | In which **field** is the **hunt** held? |
| **Gold answer** | *Main Dining Room* |
| **Zero-shot answer** | *"hunting fields"* |
| **Fine-tuned answer** | **Unable to determine** (all windows) |
| **Document page** | `doc_2.jpg` |

**Why it illustrates the thesis:** The document is a Pheasant Hunt itinerary (Thunderstik Lodge, South Dakota) listing dinner at the "Main Dining Room" and a daytime departure to "hunting fields". The C2 corruption replaces both the event ("dinner" → "hunt") and the spatial concept ("where" → "in which field"), producing a question whose answer is never explicitly stated in any structured field of the document. The zero-shot model reads "hunting fields" from the itinerary text and returns it as a plausible answer — hallucinating a match for a question the document never poses. The fine-tuned model correctly refuses across all windows.

---

## Sample 3 — Element Corruption (Category Label) | Phi-4 | SlideVQA | C1
...
## Sample 5 — Element Corruption (Acronym) | Qwen2.5-VL | MPDocVQA | C1
...
## Sample 6 — Layout Corruption (Spatial Reference) | Qwen2.5-VL | BoundingDocs | C1
...
- ZS (Gemma4): `gemma4_all_val_300/{dataset}/zeroshot_ocr/predictions.json`
...
Selection criteria:
1. ZS answer does not contain "Unable to determine" in ≥30% of windows
2. FT refuses in **all** windows
3. Page image exists on `/mnt/beegfs/amartinelli/`
4. Coverage: entity / element / layout corruption types × DUDE / MPDocVQA / BDocs datasets × Qwen2.5-VL / Gemma4 models
