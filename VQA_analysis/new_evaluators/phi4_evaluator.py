...
class Phi4VQAEvaluator:
    def __init__(self, config_path):
        with open(config_path) as f:
            self.config = json.load(f)

        self.model_config = self.config["open_source_models"]["phi4"]
    ...
    def _create_prompt(self, image_tokens, question, ocr_text=None):
        """Build the full text prompt with <|image_N|> tokens embedded at the start."""
    ...
    def generate_answer(self, question, image_paths, ocr_text=None, few_shot_turns=None, few_shot_images=None):
        """Generates model responses using a sliding-window image context.

        Phi-4 embeds images via globally-indexed <|image_N|> tokens in the text.
        few_shot_images offsets the window image indices so every token in the full
        conversation (few-shot turns + current question) points to a unique image.
        """
        try:
            ...
            all_responses = []
            # Window indices are offset by the number of few-shot images already in context
            window_img_offset = len(few_shot_images) if few_shot_images else 0
            ...
            for window in windows:
                ...
                image_tokens = "".join(
                    f"<|image_{window_img_offset + i + 1}|>" for i in range(len(window))
                )
                question_prompt = self._create_prompt(image_tokens, question, batch_ocr)
                ...
                inputs = self.processor(prompt, all_pil_images, return_tensors="pt").to("cuda")
    ...
    def _build_few_shot_turns(self, shots):
        """Returns (turns, pil_images) for Phi-4.

        Image tokens are globally numbered from 1 across all few-shot turns.
        generate_answer then offsets window image indices by len(pil_images) so
        the token sequence remains globally unique across the full conversation.
        """
        turns = []
        all_pil_images = []
        img_idx = 1  # global counter, incremented across shots
        ...
        for shot in shots:
            ...
            image_tokens = "".join(f"<|image_{img_idx + i}|>" for i in range(len(image_paths)))
            img_idx += len(image_paths)

            prompt = self._create_prompt(image_tokens, q_text, ocr_text)
            turns.append({"role": "user", "content": prompt})
            turns.append({"role": "assistant", "content": ans_text})
    ...
    def evaluate(self):
        try:
            print("\nStarting Phi-4 multimodal evaluation...")
...
def main():
    ...
    Phi4VQAEvaluator(args.config_path).evaluate()
